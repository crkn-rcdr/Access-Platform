...
<script lang="ts">
	...
	async function sendLookupRequestSlugs() {
		...
		if (slugList.length) {
			...
			error = '';
			try {
				const response = await $session.lapin.query('dipstaging.listFromSlugs', slugList);
				if (response) results = response;
			} catch (e) {
				error = 'Could not fetch package(s.) Please contact the platform team for assistance.';
			}
		}
		loading = false;
	}
	...
	onMount(async () => {
		const slugListStrPrm = $page.url.searchParams.get('slugs');
		if (slugListStrPrm) {

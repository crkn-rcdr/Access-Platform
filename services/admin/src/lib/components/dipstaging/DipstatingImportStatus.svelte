...
{#if 'smelt' in item && item['smelt'] !== null}
	<table class="table">
		<tbody>
			...
			<tr>
				...
				<td>
					{item.smelt?.requestDate ? new Date(item.smelt?.requestDate).toLocaleString() : 'N/A'}
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					<XmlViewer xml={item.smelt?.['message']?.length ? item.smelt?.['message'] : 'N/A'} />
				</td>
			</tr>
		</tbody>
	</table>
	...
	<br />
	<button class="btn secondary" on:click={clearLastImportStatus}> Clear Last Import Status </button>
	<br />

...
<script lang="ts">
	...
	function toggleAllSelected(event) {
		for (const item of items) {
			if (slugAvailableMap[slugMap[item['id']]]) selectedMap[item.id] = event.target.checked;
	...
	function setSelectedModel() {
		if (!items) return;
		for (const item of items) {
			if (!(item.id in selectedMap) && isItemSelectable(item)) selectedMap[item.id] = false;
		}
		selectedMap = selectedMap;
	}
	 ...
	 */
	$: itemsAreSelected = Object.keys(selectedMap).filter((key) => selectedMap[key]).length > 0;
</script>
								...
								<span class="status-text">
									{#if sucessfulSmeltRequestMap[legacyPackage.id]}
										This package is being imported into access! <a
											target="_blank"
											href="/smelter/queue"><br />Track its status in the 'Import Queue' tab.</a
										>
									{:else if view === 'queue'}

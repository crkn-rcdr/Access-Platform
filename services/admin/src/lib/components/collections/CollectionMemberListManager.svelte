<script lang="ts">
	...
	async function handleFillPressed() {
		...
		if (destinationCollection?.id && slugArray.length == 0) {
			error = '';
			slugArray = await $session.lapin.query('collection.getMemberSlugs', destinationCollection.id);
			...
			if (destinationCollection?.id) {
				try {
					...
					for (const result of resolutions) {
						if (result.length) {
							const slug = result[0];
							if (result.length === 2 && result[1].found && 'result' in result[1]) {
		...
		{:else}
			<div class="edit-wrap auto-align auto-align__wrap">
				<div class="member-editor">
					...
					<div class="options-wrap auto-align auto-align__wrap auto-align__a-center">
						...
						{#if action !== 1}
							...
							>
								<span slot="content"
									>{loading ? 'Loading members listing...' : 'Auto-fill Current Members'}
								</span>
							</LoadingButton>
						{/if}
					</div>
					...
					{#if action === 1}
						<br />
						<div class="row-2 auto-align auto-align__a-center">
							<label>
								<input type="radio" bind:group={addOption} name="addOption" value={1} />
								At beginning of list
							</label>

							<label>
								<input type="radio" bind:group={addOption} name="addOption" value={2} />
								At end of list
							</label>

							<label>
								<input type="radio" bind:group={addOption} name="addOption" value={3} />
								Before slug
							</label>

							<label>
								<input type="radio" bind:group={addOption} name="addOption" value={4} />
								After slug
							</label>

							{#if addOption === 3 || addOption === 4}
								<input placeholder="Enter slug" bind:value={refSlug} />{/if}
						</div>
					{/if}

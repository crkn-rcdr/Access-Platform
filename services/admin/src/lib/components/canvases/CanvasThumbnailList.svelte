...
<script lang="ts">
   ...
   */
	...
	function setPositions() {
		positions = [];
		for (let i = 0; i < canvases.length; i++) positions.push((page - 1) * size + i + 1);
	}
	...
	async function deleteCanvasByIndex(event: any, index: number) {
		...
		if (index >= 0 && index < canvases.length) {
			...
			await showConfirmation(
				async () => {
					try {
						const response = await $session.lapin.mutation('manifest.removeCanvases', data);
						return {
							success: true,
							details: ''
						};
	...
	async function sendMoveRequest(canvasToMove, pagedDestinationIndex) {
		...
		await showConfirmation(
			async () => {
				try {
					const response = await $session.lapin.mutation('manifest.moveCanvases', data);
					return {
						success: true,
						details: ''
					};
	...
	async function handleItemDropped(event: {
		...
		if (loading) return;
		loading = true;
		if (event.detail.currentItemIndex >= 0 && event.detail.currentItemIndex < canvases.length) {
			const pagedDestinationIndex = (page - 1) * size + event.detail.destinationItemIndex;
...
<div class="auto-align auto-align__full auto-align auto-align__column">
	{#if showAddButton}
		<button class="btn primary btn-lg" on:click={addClicked}>Add Canvases</button>
	{/if}

	<div class="canvas-wrap" class:disabled={loading}>
		<!-- loop through the array where items are added when scrolling -->
		<DynamicDragAndDropList bind:container={list} on:itemDropped={handleItemDropped}>
					...
					>
						<div class="actions-wrap" class:not-loading={!loading}>
							...
							>
								...
								<div class="action icon" on:click={(e) => deleteCanvasByIndex(e, i)}>
									<TiTrash />
								</div>
							</div>
						</div>
